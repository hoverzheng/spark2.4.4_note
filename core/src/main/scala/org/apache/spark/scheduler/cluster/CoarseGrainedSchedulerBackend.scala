...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  // 来自cluster manager请求但还没有注册的executor的数量
  @GuardedBy("CoarseGrainedSchedulerBackend.this")
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receive: PartialFunction[Any, Unit] = {
      case StatusUpdate(executorId, taskId, state, data) =>
        ...
        // 若Task状态是：FINISHED, FAILED, KILLED, LOST中的一种。
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case RegisterExecutor(executorId, executorRef, hostname, cores, logUrls) =>
        if (executorDataMap.contains(executorId)) {
          ...
          // 把注册的Executor，添加到executorDataMap中
    ...
    private def makeOffers() {
      ...
      val taskDescs = withLock {
        ...
        // 查看每个worker上的空闲资源，并把task分配给这些空闲资源，并生成taskdescription对象。
        scheduler.resourceOffers(workOffers)
      }
      // 把task描述发送给task对应worker的executor，开始执行task
      // task描述队列不为空
