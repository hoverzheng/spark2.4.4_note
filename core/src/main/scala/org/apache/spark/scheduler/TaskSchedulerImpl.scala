...
private[spark] class TaskSchedulerImpl(
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
      ...
      // 把该stage的所有存在的TaskSetManager都标记为zombie，当我们添加一个tm时。
      // 这对于处理一些边缘事件是有必要的。比如，有10个分区，有两个tm，一个tm是zombie一个是tm2是active的。
      // tm1有一个任务运行分区10，并且完成了。tm2完成分区1-9，感谢tm2是active的，因为它并没有完成。
      // 这样DAGScheduler会重新发送该任务。
      // 作为一个stage，不能有大于一个active的tsm，我们需要把tsm2设置成
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    // 剩下的是可用的资源列表，对可用资源进行shuffle操作，避免总是把task放到一个worker节点上
    val shuffledOffers = shuffleOffers(filteredOffers)
    // Build a list of tasks to assign to each worker.
    // 总的cpu数/每个task使用的cpu数，得到可用cpu数（也就是可以发送的task数）
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores / CPUS_PER_TASK))
    val availableCpus = shuffledOffers.map(o => o.cores).toArray
    // 可以提交的task数，可以提交的task数，是由空闲的cpu数决定
    val availableSlots = shuffledOffers.map(o => o.cores / CPUS_PER_TASK).sum
    // 按指定的调度算法对调度对象(TaskSetManager)进行排序
    val sortedTaskSets = rootPool.getSortedTaskSetQueue
    for (taskSet <- sortedTaskSets) {
      logDebug("parentName: %s, name: %s, runningTasks: %s".format(
        taskSet.parent.name, taskSet.name, taskSet.runningTasks))
      if (newExecAvail) {
        // 有新的可用executor，需要添加到taskSet
    ...
    // TaskSet已经按调度算法排好序了，从排好序的队列中获取一个taskset。
        ...
        Option(taskIdToTaskSetManager.get(tid)) match {
          case Some(taskSet) =>
            ...
            if (TaskState.isFinished(state)) {
              ...
              if (state == TaskState.FINISHED) { // Task正确的执行完成
                taskResultGetter.enqueueSuccessfulTask(taskSet, tid, serializedData)
              } else if (Set(TaskState.FAILED, TaskState.KILLED, TaskState.LOST).contains(state)) {
                // 任务是以失败的状态结束，需要处理失败状态的Task
...
private[spark] object TaskSchedulerImpl {
  ...
  // 若启用了黑名单机制，则会创建一个BlacklistTracker对象
