 ...
 */
// BlacklistTracker为了跟踪有问题的executor和worker节点。
// 它支持将整个spark应用的executor和worker节点列入黑名单（带有过期周期）。
// TaskSetManager为单个Task和Stage添加了executor和worker的其他黑名单，这些黑名单与此处的黑名单配合使用。

// 跟踪器需要处理各种工作负载的情况，例如：
// 1) 错误的用户代码-这可能会导致许多任务失败，但这不应该计入单个执行程序中
// 2) 许多小的Stage-这可以防止有问题的executor在一个stage内发生许多故障，但在整个应用程序中仍然存在许多故障
// 3) “片状”执行者-他们不会完成所有任务，但仍然有缺陷，值得列入黑名单
