...
private[spark] object SchedulerBackendUtils {
   ...
   */
  // 获取初始的executor的数量，该数量可能基于动态分配或用户指定。
  def getInitialTargetExecutorNumber(
      conf: SparkConf,
      numExecutors: Int = DEFAULT_NUMBER_EXECUTORS): Int = {
    if (Utils.isDynamicAllocationEnabled(conf)) { // 动态分配：设置了参数：spark.dynamicAllocation.enabled，且不是本地master
      ...
      initialNumExecutors
    } else { // 默认启动2个executor，否则使用用户参数：spark.executor.instances指定的个数
