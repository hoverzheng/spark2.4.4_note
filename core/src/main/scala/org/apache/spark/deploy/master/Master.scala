...
private[deploy] class Master(
  ...
  override def onStart(): Unit = {
    ...
    // standalone模式下可以通过rest接口来提交任务，若是启用，则创建相应的对象
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case RegisterWorker(
      ...
      } else if (idToWorker.contains(id)) { // 有相同的worker id
        workerRef.send(RegisterWorkerFailed("Duplicate worker ID"))
      } else { // 进入正式的注册流程
        val worker = new WorkerInfo(id, workerHost, workerPort, cores, memory,
          workerRef, workerWebUiUrl)
        if (registerWorker(worker)) { // worker注册成功
    ...
    case RegisterApplication(description, driver) =>
      ...
      if (state == RecoveryState.STANDBY) { //目前master的状态是standby，则什么也不做
  ...
  private def schedule(): Unit = {
    ...
    // 遍历目前正在等待的app，为这些app启动executor，用来执行task
...
private[deploy] object Master extends Logging {
  ...
  def startRpcEnvAndEndpoint(
    ...
    // 创建并启动一个transportServer对象，该对象用来在shuffle时传输数据
    val rpcEnv = RpcEnv.create(SYSTEM_NAME, host, port, conf, securityMgr)
    // 创建Master对象，并创建一个RpcEndpoint，名称为：Master
