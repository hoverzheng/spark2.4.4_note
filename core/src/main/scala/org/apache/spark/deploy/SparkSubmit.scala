...
private[spark] class SparkSubmit extends Logging {
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    // 有几类的cluster模式是不支持的: local，standalone+python, standalone+r
    ...
    // 简化一下clustermanager和部署模式的判断，后面判断方便
    ...
    // 非mesos cluster或standalone cluster模式
    ...
    // 若添加了--principal和--keytab选项，则需要对用户进行认证
    ...
    // 若是客户端模式，则需要把ftp,http等服务器上的文件下载到本地。若是本地文件，什么都不做
    var localPrimaryResource: String = null
    var localJars: String = null
    var localPyFiles: String = null
    if (deployMode == CLIENT) {
      // 需要运行的类的主jar包，可以指定ftp或http，此时此时需要把它们下载到本地
      // 处理命令行最后的jar包，这是保存主类的地方
      localPrimaryResource = Option(args.primaryResource).map {
        // 注意：若没有设置前缀，比如：file://,http,ftp,hdfs等，默认会在路径前面添加file://
    ...
    // 运行python 应用程序 且是 客户端模式，同样需要启动主类。
    if (args.isPython && deployMode == CLIENT) {
      if (args.primaryResource == PYSPARK_SHELL) { // 若是pyspark-shell，主类是下面的
        args.mainClass = "org.apache.spark.api.python.PythonGatewayServer"
      } else { // 否则主类是PythonRunner
    ...
    // 若是client模式，则直接运行应用程序的主类。注意：无论是哪种语言此时主类都已经确定
    if (deployMode == CLIENT) {
    ...
    // standalone cluster模式，主类是固定的：
    if (args.isStandaloneCluster) {
      if (args.useRest) { // 通过rest接口提交任务
    ...
    // 若是yarn-cluster模式，主类是固定的:org.apache.spark.deploy.yarn.YarnClusterApplication
    if (isYarnCluster) {
    ...
    // 若是集群模式，忽略spark.driver.host参数的值
  ...
  private def runMain(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    ...
    // 创建SparkApplication对象，实现start()函数 。
    // 若实现了SparkApplication接口，则直接使用，否则进行封装。
