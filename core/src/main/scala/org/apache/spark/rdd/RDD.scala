...
abstract class RDD[T: ClassTag](
   ...
   * RDD is checkpointed.
   */
  // 获取分区的首选位置，需要考虑是否RDD被checkpoint
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    // 已经设置了存储级别，直接从缓存的地方获取。可能是通过persist或cache等操作来设置的。
    if (storageLevel != StorageLevel.NONE) {
      getOrCompute(split, context)
    } else {
      // 若没有设置storagelevel，则需要重新计算
   ...
   */
  // 该函数会直接返回一个具有确定分区的rdd。实现层面，通过一个shuffle过程来重新分配数据。
  // repartition会产生shuffle，若是需要减少数据的分区，最好直接使用coalesce函数
   ...
   */
  // 当rdd在一个阻碍stage中。spark必须在同一时刻发送一个阻隔stage中的所有task。
  // 若一个rdd在一个阻隔stage中，则它的父rdd或它自己，被映射到RDDBarrier.一个ShuffleRDD代表新的stage的开始。
  // RDD是否在一个被阻碍的stage中。spark必须为一个阻碍的stage，在同一个时间点，提交所有的任务。
  private[spark] def isBarrier(): Boolean = isBarrier_

  // From performance concern, cache the value to avoid repeatedly compute `isBarrier()` on a long
  // RDD chain.
  // 出于性能的考虑，为了避免重复计算一个长的RDD链，需要缓存数据。非shuffle依赖
