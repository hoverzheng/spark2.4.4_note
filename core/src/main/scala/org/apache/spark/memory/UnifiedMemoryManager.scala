...
private[spark] class UnifiedMemoryManager private[memory] (
  ...
  override private[memory] def acquireExecutionMemory(
     ...
     */
    // 增加执行内存池。通过驱逐缓存的数据块，因此会缩减存储内存池。

    // 当为任务获取执行内存时，执行内存池可能进行多次尝试。
    // 每次必须要能够释放一些存储，以防其他任务进入，并在两次尝试之间缓存较大数据块。
    def maybeGrowExecutionPool(extraMemoryNeeded: Long): Unit = {
      if (extraMemoryNeeded > 0) {
        ...
        // 若存储池有空闲内存，或则存储池借用了执行池的内存，就执行内存分配的动作
        if (memoryReclaimableFromStorage > 0) {
          // Only reclaim as much space as is necessary and available:
          // 不会按最大的空闲内存来申请，只有能满足最小的内存大小即可
  ...
  override def acquireStorageMemory(
    ...
    // 若申请内存量大于存储池的空闲内存量，此时需要向执行池借用内存
    if (numBytes > storagePool.memoryFree) {
      // There is not enough free memory in the storage pool, so try to borrow free memory from
      // the execution pool.
      // 计算从执行池借用的量，最多只能借用，执行池空余的内存量
