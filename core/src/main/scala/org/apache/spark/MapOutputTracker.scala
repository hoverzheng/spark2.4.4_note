...
// 创建一个MapOutputTrackerMaster，运行在driver端
 ...
 * test code.
*/
// 跟踪stage的map输出的位置。这是一个抽象类，driver和exeutor有不同的实现版本。
...
private[spark] class MapOutputTrackerMaster(
   ...
   */
  // 返回最佳的主机，该主机用来运行给定map输出的分区，在给定的shuffle过程中。
  // 例如，在这些 节点上，对于每个分区有大部分分区输出。
