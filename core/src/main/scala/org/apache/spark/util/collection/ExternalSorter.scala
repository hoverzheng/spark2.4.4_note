...
private[spark] class ExternalSorter[K, V, C](
  ...
  private def getPartition(key: K): Int = { // 获取key所在的分区号
  ...
  @volatile private var map = new PartitionedAppendOnlyMap[K, C]  // 有聚合操作时使用
  @volatile private var buffer = new PartitionedPairBuffer[K, C]  // 没有聚合操作时使用
  ...
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    // todo: 停止合并，若我们发现reduce因子太高。

    val shouldCombine = aggregator.isDefined

    if (shouldCombine) { // 若需要进行排序（聚合）
      // Combine values in-memory first using our AppendOnlyMap
      // 首先在内存中对数据进行聚合：执行聚合函数
      ...
      while (records.hasNext) {
        addElementsRead()
        val kv = records.next()
        // 把数据放到buffer中，这里其实是放到一个数组中，当数组满了时，会创建一个原来2倍容量的数组，然后把原数组的数据复制过去。
   ...
   */
  // 若需要的话（），把目前内存集合中的数据写入到磁盘。
  private def maybeSpillCollection(usingMap: Boolean): Unit = {
    var estimatedSize = 0L
    if (usingMap) { // 使用map还是buffer作为缓存数据的结构
      estimatedSize = map.estimateSize()
      // 内存到达上限，把目前内存中的数据写入磁盘中
   ...
   */
  // 把目前内存collection中的内容写入一个已经排序的文件中，该文件后面会进行合并。
