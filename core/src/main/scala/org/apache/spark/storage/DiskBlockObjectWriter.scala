 ...
 * reopened again.
 */
// 该类把JVM对象直接写入到磁盘文件。该类允许把数据追加到一个存在的数据块。
// 为了提升效率，该类会一致保持文件channel供多次提交使用，直到close()被调用才关闭channel。
// 若可能出现错误，调用者应该使用revertPartialWritesAndClose()函数，它可以还原（回滚）没有写入到磁盘的部分数据。
private[spark] class DiskBlockObjectWriter(
  ...
  def commitAndGet(): FileSegment = {
    if (streamOpen) {
      ...
      if (syncWrites) { //默认为false，通过参数spark.shuffle.sync来进行配置
