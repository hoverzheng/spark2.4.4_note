 ...
 */
// 接口，抽象如何存储数据块。并提供不同的方式来读取块数据。
// 当完成数据块的操作后，调用者应该调用dispose函数来清理中间缓存。
 ...
 */
// 数据块管理器，运行在每个节点上（包括driver和executor节点）。
// 它提供从本地和远端获取和写入数据的接口，这些数据可能保存在各种存储系统（内存，磁盘，栈外内存）。
private[spark] class BlockManager(
  ...
  // executor中的shuffle文件服务的地址，可能是外部服务或executor端的blockManager，其实是一个BlockManagerId对象
  private[spark] var shuffleServerId: BlockManagerId = _

  // Client to read other executors' shuffle files. This is either an external service, or just the
  // standard BlockTransferService to directly connect to other Executors.
  // 从其他executor中读取shuffle文件的Client端。这可能给是一个外部服务（为false，不启用），
  // 或则是一个可以直接连接到其他executor的标准BlockTransferService。
  ...
  def initialize(appId: String): Unit = {
    ...
    // 若shuffleClient也就是没有启用外部shuffle服务（参数spark.shuffle.service.enabled，默认为false)
    // 没有启动外部shuffle服务时shuffleClient=blockTransferService，此时init什么都不做
    ...
    // 初始化BlockManager的唯一标识类，该类保存了端口，主机名，拓扑等信息
    val id =
      BlockManagerId(executorId, blockTransferService.hostName, blockTransferService.port, None)

    // 向driver端的BlockManager注册，获取BlockManagerId的拓扑信息
    ...
    // 若启用了外部服务（默认是不启用），且不是driver端，向本地shuffle服务注册executor的配置信息
  ...
  override def getBlockData(blockId: BlockId): ManagedBuffer = {
    if (blockId.isShuffle) { // 若是shuffle产生的文件
   ...
   * memory on slave.
   */
  // 告诉master(driver)端，目前的块数据的存储情况。
  // 不会按该块信息中的存储级别(storageLevel)来发送块信息，而是发送一个块更新消息，用来反映目前的状态。
   ...
   */
  // 从本地blockManager获取数据块，并作为Java对象的迭代器
  def getLocalValues(blockId: BlockId): Option[BlockResult] = {
    ...
    blockInfoManager.lockForReading(blockId) match {
      ...
      case Some(info) =>
        ...
        if (level.useMemory && memoryStore.contains(blockId)) {  // block在内存中
        ...
        } else if (level.useDisk && diskStore.contains(blockId)) { // block在磁盘中
   ...
   */
  // 以字节序列化的形式，从本地的blockmanager获取数据块
   ...
   */
  // 从远端的blockManager获取数据块，该操作不会获取锁
   ...
   */
  // 从远端的blockManager获取块数据，并按序列化字节的形式返回。
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    // 从driver端获取数据块的状态和位置信息。
    // 由于所有远端的数据块都已经在driver端注册，所以，没有必要请求所有的slave executor端去获取块的状态。?
    ...
    // 为给定的块选择一个位置，优先选择本机，因为多个blockManager可能共享主机。
   ...
   */
  // 从blockmanager获取一个数据块（本地或远端都可以）
  // 若是从本地获取数据块，需要获取一个读锁，若从远端读取不需要获取锁。
  // 读锁在结果数据迭代器被消费完后自动释放。
   ...
   * @return true if the block was stored or false if an error occurred.
   */
  // 把一个序列化字节的新数据块，保存到blockManagger
   ...
   */
  // 把给的字节数据保存到blockManagger，需要根据StoreageLevel来进行保存，若需要备份还需要复制该数据。
  // 若数据块已经存在，该函数不会覆盖它。
  // 注意：调用者不能修改或释放保存字节数据的buffer。
  private def doPutBytes[T](
    ...
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      ...
      val replicationFuture = if (level.replication > 1) { // 复制大于1
        Future {
          // This is a blocking action and should run in futureExecutionContext which is a cached
      ...
      if (level.useMemory) { // 内存level
        // Put it in memory first, even if it also has useDisk set to true;
        // We will drop it to disk later if the memory store can't hold it.
        val putSucceeded = if (level.deserialized) { // 非序列化对象模式
          val values =
        ...
        // 在内存中没有保存成功，且也设置了使用磁盘存储，则需要把数据保存到磁盘上
        if (!putSucceeded && level.useDisk) {
          logWarning(s"Persisting block $blockId to disk instead.")
          diskStore.putBytes(blockId, bytes)
        }
      } else if (level.useDisk) { // 磁盘level
        diskStore.putBytes(blockId, bytes)
      }
      ...
      if (blockWasSuccessfullyStored) {
        ...
        info.size = size
        // 把该块数据最新的状态汇报给driver，该block已经在内存或磁盘中
      ...
      if (level.replication > 1) {  // 若复制数据块
        // Wait for asynchronous replication to finish
        try { // 启动复制线程复制数据块到其他节点上。调用blockTransferService.uploadBlockSync来实现块数据的复制。
   ...
   */
  // 把一个块数据复制到其他节点。注意：这是一个阻塞调用，将会在块复制完成后返回。
