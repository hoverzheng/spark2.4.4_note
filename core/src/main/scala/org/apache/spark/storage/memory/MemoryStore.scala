...
// 内存中的存储对象，由三个部分组成：大小，内存模式，类标识
private sealed trait MemoryEntry[T] {
  def size: Long
  def memoryMode: MemoryMode
  def classTag: ClassTag[T]
}
// 非序列化内存存储对象，只能保存在JVM的堆内内存中，以数组的方式保存
...
// 序列化内存存储对象，可以使用堆内和堆外内存保存，通过ChunkedByteBuffer结构来进行保存。
...
private[storage] trait BlockEvictionHandler {
   ...
   */
  // 从内存中删除一个数据块，如果可能，会把它保存到磁盘上。
  // 当内存的存储到达上限时，需要释放内存空间。
 ...
 */
// 在内存数据块，或则作为反序列化的java对象，或则作为序列化后的byte数组。
   ...
   */
  // 尝试释放内存块，而存储特定的块。可能会失败，若数据块的比内存块大。
    ...
    memoryManager.synchronized {
      ...
      entries.synchronized {
        ...
        while (freedMemory < space && iterator.hasNext) {
          ...
          // 数据块是否可以被驱逐，条件为：（1）内存模式相同（2）不是相同的rdd 或（2）非RDD的数据块
      ...
      if (freedMemory >= space) {
        ...
        try {
          ...
          (0 until selectedBlocks.size).foreach { idx =>
            ...
            // future safety.
            // 把blockId对应的数据块从内存中删除，若使用了useDisk，则会把这些数据写到磁盘上。
            if (entry != null) {
