 ...
 */
// 该类用来维护：逻辑数据块和磁盘的物理位置的对应关系。
// 一个BlockId标识的数据块对应一个文件。
private[spark] class DiskBlockManager(conf: SparkConf, deleteFilesOnStop: Boolean) extends Logging {
  ...
  // 为shuffle的中间结果创建一个文件：创建一个blockId和File对象的对应关系，用来保存shuffle的中间结果
   ...
   */
  // 为存储块数据创建本地路径。当使用外部shuffle服务时，这些本地路径不会被删除。
