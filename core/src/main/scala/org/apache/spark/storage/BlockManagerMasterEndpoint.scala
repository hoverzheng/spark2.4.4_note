...
class BlockManagerMasterEndpoint(
  ...
  private def heartbeatReceived(blockManagerId: BlockManagerId): Boolean = {
    if (!blockManagerInfo.contains(blockManagerId)) { // 某个blockmanagerId不存在时，若该Id是driver端且不是local模式返回true。
  ...
  private def register(
    ...
    // 查看该拓扑图中是否包含前来注册的主机id
    if (!blockManagerInfo.contains(id)) { // 不包含该BlockManagerId
      blockManagerIdByExecutor.get(id.executorId) match { //blockManagerId相同，但executorId不同
        case Some(oldId) =>
  ...
  private def updateBlockInfo(
    ...
    if (!blockManagerInfo.contains(blockManagerId)) {
      if (blockManagerId.isDriver && !isLocal) { //是driver端，且非local模式，返回true。
