 ...
 */
// 用来获取多个数据块的迭代器。对于本地块，它从本地的blockManager获取。对于远端的数据块，它通过提供的BlockTransferService服务来读取。
// 该类创建了一个(BlockID, InputStream)元组的迭代器，这样，在接收到数据块时，调用者可以在一个pipeline中进行处理。
// 为了避免使用过多的内存，该实现会限制从远端获取的数据量，使得它不超过maxBytesInFlight的值。
private[spark]
final class ShuffleBlockFetcherIterator(
   ...
   */
  // 正在被处理的FetchResult，跟踪这些结果数据的处理，一旦出现异常可以释放内存
   ...
   */
  // 将要发送的获取数据请求的队列。我们将会逐渐处理该队列的请求，确保传输中的字节数不超过：maxBytesInFlight。
   ...
   */
    // 获取数据的请求的队列，该队列是：第一次从队列取出后不能被发送的请求。当发送条件满足时，他们会再次被发送。
